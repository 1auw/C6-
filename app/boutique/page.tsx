...
import { ShoppingCart, Filter, Car, Home, Package, Wrench, Crown, X, ChevronDown, Lock, Tag } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#2a7cff]/10 rounded-full blur-[150px]" />
      ...
      <AnimatePresence>
        {showLoginModal && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white">Connexion requise</h3>
                <button
                  onClick={() => setShowLoginModal(false)}
              ...
              <p className="text-gray-400 mb-6">
                Connectez-vous a votre compte Central6RP pour effectuer des achats.
              </p>
              <div className="flex gap-4">
                <Link href="/login" className="flex-1">
                  <button className="w-full bg-[#2a7cff] hover:bg-[#1e5fd4] text-white font-semibold py-3 px-6 rounded-lg transition-colors">
                    Se connecter
                  </button>
                </Link>
                <Link href="/register" className="flex-1">
                  <button className="w-full bg-white/10 hover:bg-white/20 text-white font-semibold py-3 px-6 rounded-lg transition-colors border border-white/20">
                    S'inscrire
                  </button>
                </Link>
              </div>
...
function ProductCard({
  ...
  return (
    <div className={`bg-[#111]/80 backdrop-blur-sm border rounded-xl overflow-hidden group hover:border-[#2a7cff]/50 transition-all duration-300 ${
      featured ? "border-[#2a7cff]/30" : "border-white/10"
    }`}>
      {/* Image placeholder with gradient */}
      <div className="relative h-40 bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] flex items-center justify-center overflow-hidden">
        ...
        {featured && (
          <div className="absolute top-12 left-3 z-20">
            <span className="bg-[#2a7cff] text-white text-xs font-bold px-2 py-1 rounded">
              EN AVANT
            </span>
          </div>
        )}
        ...
        <div className="relative z-5 text-white/10 group-hover:text-white/20 transition-colors">
          {product.category === "VIP" && <Crown size={64} />}
          {product.category === "IMMOBILIER" && <Home size={64} />}
          {product.category === "PACK_DEPART" && <Package size={64} />}
          {product.category === "SERVICE" && <Wrench size={64} />}
          {product.category === "VEHICULE" && <Car size={64} />}
        </div>
      </div>

      {/* Content */}
      <div className="p-4">
        <h3 className="text-white font-semibold mb-2 line-clamp-2 min-h-[48px]">
          {product.name}
        </h3>
        ...
        <div className="text-xs text-gray-500 mb-3">
          Achat lie a votre compte Central6RP
        </div>
